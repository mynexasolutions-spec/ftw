        ...
        <div>
          <h2 className="font-sans text-2xl font-black uppercase tracking-tight text-dark flex items-center gap-3">
            <div className="p-2 bg-dark text-white rounded-xl shadow-md">
              <Palette className="w-5.5 h-5.5 text-accent" />
            </div>
            Customizer Mockups & Colors
          </h2>
          <p className="text-xs text-dark2/50 mt-1 font-medium">Define active blank t-shirt mockups and colors available inside the customer Studio Customizer tool.</p>
        </div>
        <button
          onClick={handleSaveConfig}
          disabled={saving}
          className="px-6 py-3 bg-dark hover:bg-accent hover:text-dark text-cream font-sans font-bold uppercase tracking-widest text-[10px] rounded-xl shadow-md flex items-center justify-center gap-2 border-none cursor-pointer transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 shrink-0"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div id="color-form-card" className="bg-white border border-cream3 rounded-3xl p-6 shadow-sm space-y-5">
            <span className="text-[10px] text-accent font-black uppercase tracking-widest block border-b border-cream3 pb-2 font-mono">
              {editingColorName ? `1. Edit Customizer Color: ${editingColorName}` : '1. Define New Customizer Color Mockups'}
            </span>
            <form onSubmit={handleAddColor} className="space-y-5 text-xs font-sans">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase tracking-wider block font-bold text-dark2/60">Color Name *</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-4 py-2.5 bg-cream/35 border border-cream3 rounded-xl focus:outline-none focus:border-dark focus:bg-white text-xs font-sans font-bold transition-all placeholder-dark/30"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] uppercase tracking-wider block font-bold text-dark2/60">Color Hex Code *</label>
                  <div className="flex gap-2">
                    <input
                      type="color"
                      value={newColorHex}
                      onChange={(e) => setNewColorHex(e.target.value)}
                      className="w-10 h-9 p-0.5 bg-white border border-cream3 rounded-xl cursor-pointer"
                    />
                    <input
                      type="text"
                      required
                      ...
                      value={newColorHex}
                      onChange={(e) => setNewColorHex(e.target.value)}
                      className="flex-1 px-4 py-2.5 bg-cream/35 border border-cream3 rounded-xl focus:outline-none focus:border-dark focus:bg-white text-xs font-sans font-mono font-bold transition-all placeholder-dark/30"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-wider block font-bold text-dark2/60 mb-2">Upload Blank Mockups *</label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {/* Front */}
                  <div className="space-y-1.5">
                    <span className="text-[9px] uppercase font-bold text-dark2/50 block text-center">Front View *</span>
                    {mockupFront ? (
                  ...
                  {/* Back */}
                  <div className="space-y-1.5">
                    <span className="text-[9px] uppercase font-bold text-dark2/50 block text-center">Back View *</span>
                    {mockupBack ? (
                      <div className="border-2 border-cream3 rounded-xl p-3.5 flex flex-col items-center justify-center bg-cream2/30 aspect-square relative overflow-hidden group">
                  ...
                  {/* Left Sleeve */}
                  <div className="space-y-1.5">
                    <span className="text-[9px] uppercase font-bold text-dark2/50 block text-center">Left Sleeve</span>
                    {mockupLeftSleeve ? (
                      <div className="border-2 border-cream3 rounded-xl p-3.5 flex flex-col items-center justify-center bg-cream2/30 aspect-square relative overflow-hidden group">
                  ...
                  {/* Right Sleeve */}
                  <div className="space-y-1.5">
                    <span className="text-[9px] uppercase font-bold text-dark2/50 block text-center">Right Sleeve</span>
                    {mockupRightSleeve ? (
                      <div className="border-2 border-cream3 rounded-xl p-3.5 flex flex-col items-center justify-center bg-cream2/30 aspect-square relative overflow-hidden group">
        ...
        <div className="bg-white border border-cream3 rounded-3xl p-6 shadow-sm space-y-5">
          <span className="text-[10px] text-accent font-black uppercase tracking-widest block border-b border-cream3 pb-2 font-mono">
            Size Chart for All Mockup T-shirts
          </span>
          <div className="text-xs font-sans space-y-4">
            <p className="text-dark2/60 text-xs">Configure the measurements (Chest, Length, Sleeve) shown to customers on the customizer page's size chart.</p>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-cream2 text-dark/65 font-bold border-b border-cream3 font-mono text-[9px] uppercase tracking-wider">
                    ...
                    <th className="p-3.5 text-accent">A (Chest Width)</th>
                    <th className="p-3.5 text-dark/85">B (Length)</th>
                    <th className="p-3.5 text-orange-500">C (Sleeve Length)</th>
                  </tr>
                </thead>
                  ...
                  {['S', 'M', 'L', 'XL', 'XXL'].map(sz => {
                    ...
                    return (
                      <tr key={sz} className="hover:bg-cream2/20">
                        <td className="p-3.5 font-bold font-mono text-dark text-sm">{sz}</td>
                        <td className="p-3.5">
                          <input
                            type="text"
                            value={szData.chest || ''}
                            ...
                            className="w-full px-3 py-1.5 border border-cream3 rounded-xl focus:outline-none focus:border-dark text-xs"
                            placeholder='e.g. 38"'
                          />
                        </td>
                        <td className="p-3.5">
        ...
        <div className="bg-white border border-cream3 rounded-3xl p-6 shadow-sm space-y-5">
          ...
          <div className="text-xs font-sans space-y-5">
            <p className="text-dark2/60 text-xs font-medium">Set the base blank T-shirt price and individual printing costs per print zone.</p>

            {/* Blank T-shirt price — full width */}
            <div className="space-y-1.5">
              <label className="text-[10px] uppercase tracking-wider block font-bold text-dark2/60">Blank T-shirt Price (₹)</label>
              <input
                type="number"
                value={pricing.customizer_blank_price}
                onChange={(e) => setPricing(prev => ({ ...prev, customizer_blank_price: Number(e.target.value) }))}
                className="w-full px-4 py-2.5 bg-cream/35 border border-cream3 rounded-xl focus:outline-none focus:border-dark text-xs font-sans font-bold"
                placeholder="e.g. 380"
              />
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-wider block font-bold text-dark2/60">Print Cost per Zone (₹)</label>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={key} className="space-y-1.5">
                    <label className="text-[10px] uppercase tracking-wider block font-bold text-dark2/50">{label}</label>
                    <input
                      type="number"
                      value={pricing[key]}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-cream3 rounded-3xl p-6 shadow-sm space-y-4">
            <span className="text-[10px] text-accent font-black uppercase tracking-widest block border-b border-cream3 pb-2 font-mono">2. Active Customizer Colors ({config.colors.length})</span>

            {config.colors.length === 0 ? (
              <div className="text-center py-8 text-dark2/40 text-xs flex flex-col items-center gap-2">
              ...
              <div className="flex flex-col gap-3 max-h-[480px] overflow-y-auto pr-1">
                {config.colors.map(col => (
                  <div key={col.name} className="flex flex-col gap-3.5 p-4 bg-cream2 rounded-2xl border border-cream3 hover:border-dark/15 hover:shadow-xs transition-all duration-300">
                    {/* Top Row: Color circle, Name/Hex, Delete Button */}
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div
                          className="w-7 h-7 rounded-full shrink-0 border border-neutral-200/50 shadow-inner"
                          style={{ backgroundColor: col.hex }}
                        />
                        <div className="min-w-0">
                          <span className="font-extrabold text-xs block text-dark leading-tight uppercase tracking-tight">{col.name}</span>
                          <span className="text-[9px] font-mono text-dark2/40 font-bold uppercase tracking-wider block mt-0.5">{col.hex}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          ...
                          className="p-1.5 bg-cream3 hover:bg-dark hover:text-white text-dark/70 rounded-xl transition-all border-none cursor-pointer flex items-center justify-center shadow-xs"
                          title="Edit Color"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white rounded-xl transition-all border-none cursor-pointer flex items-center justify-center shadow-xs"
                          title="Remove Color"
                        >
                    ...
                    {col.mockups && (
                      <div className="flex flex-wrap items-center justify-between gap-2 pt-2.5 border-t border-cream3/60">
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(col.mockups).map(([key, url]) => {
                            ...
                            return (
                              <div
                                key={key}
                                className="w-9 h-9 rounded-lg bg-white border border-cream3 overflow-hidden flex items-center justify-center p-0.5 relative group/thumb cursor-pointer hover:border-dark/25 transition-all shadow-xs"
                        ...
                        <span className="text-[8px] font-mono bg-dark/5 px-2 py-0.5 rounded border border-cream3 uppercase text-dark2/60 font-bold shrink-0">
