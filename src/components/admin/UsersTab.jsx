    ...
    <div className="space-y-8 animate-fade-in pb-12 text-dark font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-cream3 pb-5">
        <div>
          ...
          <p className="text-xs text-dark2/50 mt-1 font-medium">Manage user profiles, permissions, tracking roles, and user details.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-black text-purple-700/80 uppercase tracking-widest block">Total Accounts</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-black text-purple-950 mt-1 font-mono">{totalUsers}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-purple-600 bg-purple-50/90 px-2 py-0.5 rounded border border-purple-150/50">Registered</span>
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-black text-amber-700/80 uppercase tracking-widest block">Administrators</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-black text-amber-955 mt-1 font-mono">{adminCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-amber-600 bg-amber-50/90 px-2 py-0.5 rounded border border-amber-150/50">Staff access</span>
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-black text-blue-700/80 uppercase tracking-widest block">Clients</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-black text-blue-950 mt-1 font-mono">{customerCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-blue-600 bg-blue-50/90 px-2 py-0.5 rounded border border-blue-150/50">Shop Buyers</span>
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-black text-emerald-700/80 uppercase tracking-widest block">Active Shoppers</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-black text-emerald-950 mt-1 font-mono">{activeShoppersCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-emerald-600 bg-emerald-550/10 px-2 py-0.5 rounded border border-teal-150/50">With Orders</span>
              ...
              <table className="w-full text-left border-collapse text-xs font-sans">
                <thead>
                  <tr className="bg-cream/40 border-b border-cream3 text-[10px] uppercase font-black text-dark2/50 tracking-wider">
                    ...
                    <th className="p-4">UUID Reference</th>
                      ...
                      >
                        ...
                        <td className="p-4 pl-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="font-bold text-dark block text-xs uppercase">{userObj.name || 'Anonymous User'}</span>
                              <span className="text-[10px] text-dark2/45 font-mono block mt-0.5">{userObj.email}</span>
                            </div>
                          </div>
                        </td>
                        ...
                        {/* User UUID */}
                        <td className="p-4">
                          <div className="flex items-center gap-1.5 font-mono text-[10px] text-dark2/40">
                            <span className="truncate max-w-[120px]" title={userObj.id}>{userObj.id}</span>
                            <button 
                              ...
                              title="Copy UID"
                            >
                              {copiedUid === userObj.id ? <Check className="w-3 h-3 text-green-600" /> : <Copy className="w-3 h-3" />}
                            </button>
                          </div>
                        </td>
                        ...
                        <td className="p-4">
                          {orderCount > 0 ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-blue-50 text-blue-700 border border-blue-150 text-[9px] font-black uppercase rounded-lg">
                              <ShoppingBag className="w-3 h-3" /> {orderCount} Order{orderCount > 1 ? 's' : ''}
                            </span>
                          ) : (
                            <span className="text-dark2/35 text-[9px] font-mono">No purchases</span>
                          )}
                        </td>
