    ...
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-cream3 pb-5">
        <div>
          ...
          <p className="text-xs text-dark2/50 mt-1 font-medium font-sans">Manage customer transactions, tracking status, and custom print requests.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-sans font-black text-purple-700/80 uppercase tracking-widest block">Total Orders</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-sans font-black text-purple-950 mt-1 font-mono">{totalCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-purple-600 bg-purple-50/90 px-2 py-0.5 rounded border border-purple-150/50">All received</span>
        ...
        >
          <div>
            ...
            <p className="text-xl sm:text-2xl lg:text-3xl font-sans font-black text-amber-950 mt-1 font-mono">{pendingCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-amber-600 bg-amber-50/90 px-2 py-0.5 rounded border border-amber-150/50 animate-pulse">Awaiting dispatch</span>
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-sans font-black text-blue-700/80 uppercase tracking-widest block">Shipped Orders</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-sans font-black text-blue-950 mt-1 font-mono">{shippedCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-blue-600 bg-blue-50/90 px-2 py-0.5 rounded border border-blue-150/50">In transit</span>
        ...
        >
          <div>
            <span className="text-[9px] sm:text-[10px] font-sans font-black text-emerald-700/80 uppercase tracking-widest block">Delivered Orders</span>
            <p className="text-xl sm:text-2xl lg:text-3xl font-sans font-black text-emerald-950 mt-1 font-mono">{deliveredCount}</p>
          </div>
          <div className="flex justify-between items-center mt-2 sm:mt-3">
            <span className="text-[8px] sm:text-[9px] font-extrabold text-emerald-600 bg-emerald-550/10 px-2 py-0.5 rounded border border-teal-150/50">Completed</span>
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between bg-cream2/30 border border-cream3 p-3 sm:p-4 rounded-2xl">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 border shrink-0 snap-start ${isTabActive
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center w-full lg:w-auto">
          ...
          <div className="relative flex items-center group">
            {(() => {
              const iconClass = "absolute left-3 w-4 h-4 text-dark/45 pointer-events-none group-focus-within:text-dark transition-colors"
            ...
            <select
              ...
              className="w-full sm:w-44 pl-9 pr-8 py-2.5 bg-white border border-cream3 rounded-xl focus:outline-none focus:border-dark text-xs font-sans font-bold text-dark cursor-pointer appearance-none shadow-xs"
            >
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-dark/40 text-[10px]">▼</div>
          </div>
          ...
          <div className="relative w-full sm:w-64 group">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-cream3 bg-white hover:border-neutral-355 focus:border-dark focus:bg-white text-xs text-dark focus:outline-none font-sans font-semibold transition-all shadow-xs placeholder-dark/30"
            />
          </div>
        </div>
      </div>
                  ...
                  {finalOrdersList.map(order => {
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-cream/20 transition-colors group text-dark font-sans">
                        <td className="p-4 pl-6 font-bold text-dark font-mono truncate max-w-[120px]" title={order.id}>{order.id}</td>
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-dark uppercase block text-xs">{order.customer_name}</span>
                        ...
                        <td className="p-4">
                          <select value={order.status || 'Pending'} onChange={(e) => handleStatusChange(order.id, e.target.value)} className="px-2 py-1 bg-cream/70 hover:bg-cream border border-cream3 rounded-lg text-[10px] font-sans font-bold text-dark focus:outline-none cursor-pointer focus:border-dark transition-all">
                            <option value="Pending">Pending</option>
                            <option value="Processing">Processing</option>
                        ...
                        <td className="p-4 text-right pr-6">
                          <button onClick={() => setSelectedOrder(order)} className="p-2 bg-cream hover:bg-dark hover:text-white rounded-xl text-dark2 hover:text-white transition-all cursor-pointer border border-cream3 hover:border-dark inline-flex items-center justify-center shadow-xs">
          ...
          return (
            <div className="fixed inset-0 z-[1200] flex items-center justify-center p-3 sm:p-4">
              ...
              <motion.div
                ...
                className="bg-cream border border-cream3 w-full max-w-3xl p-4 sm:p-6 rounded-3xl shadow-2xl relative z-10 font-sans text-dark max-h-[90vh] overflow-y-auto"
              >
                {/* Close Button */}
                <button
                  onClick={() => setSelectedOrder(null)}
                ...
                <div className="border-b border-cream3 pb-4 mb-4 pr-10">
                  <span className="text-[9px] font-mono text-dark2/45 uppercase tracking-widest font-black block mb-0.5">MANAGEMENT PANEL</span>
                  <h3 className="font-sans text-lg sm:text-xl md:text-2xl font-black uppercase text-dark">Order Overview</h3>
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-1.5">
                    <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase border ${isCancelled ? 'bg-red-500/10 text-red-700 border-red-150' : selectedOrder.status === 'Delivered' ? 'bg-green-550/10 text-green-700 border-green-150' : selectedOrder.status === 'Shipped' || selectedOrder.status === 'Dispatched' ? 'bg-blue-550/10 text-blue-700 border-blue-150' : selectedOrder.status === 'Processing' ? 'bg-purple-550/10 text-purple-700 border-purple-150' : 'bg-yellow-555/10 text-yellow-700 border-yellow-150'}`}>{selectedOrder.status || 'Pending'}</span>
                    <button
                      onClick={() => handleCopy(selectedOrder.id, 'id')}
                      className="text-[10px] sm:text-[11px] font-mono font-bold text-dark/70 hover:text-dark flex items-center gap-1.5 bg-cream2 px-2 py-0.5 rounded border border-cream3 transition-all cursor-pointer"
                    >
                      <span>ID: {selectedOrder.id}</span>
                      {copiedField === 'id' ? <Check className="w-3 h-3 text-green-600 animate-bounce" /> : <Copy className="w-3 h-3 text-dark2/50" />}
                    </button>
                  </div>
                </div>

                {/* Master Layout Grid */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-5 text-xs text-dark">
                  ...
                  <div className="lg:col-span-7 space-y-5">
                    ...
                    {isCancelled && (
                      <div className="flex items-center gap-3 p-3 bg-red-50 border border-red-100 text-red-700 rounded-xl text-xs font-semibold">
                        <X className="w-5 h-5 shrink-0 bg-red-100 p-1 rounded-full" />
                        <div>
                          <p className="font-bold uppercase text-[10px]">Cancelled Order</p>
                          <p className="text-[10px] text-red-650/80 mt-0.5">This transaction has been terminated and stock allocations reverted.</p>
                        </div>
                      </div>
                    )}

                    {/* Items Overview */}
                    <div className="bg-white border border-cream3 rounded-2xl p-4 sm:p-5">
                      <span className="text-[9.5px] text-dark2/45 uppercase font-black tracking-widest block border-b border-cream3 pb-2 mb-3">Items Overview ({selectedOrder.items?.length || 0})</span>
                      <div className="space-y-3.5 max-h-[220px] overflow-y-auto pr-1 scrollbar-thin">
                        {selectedOrder.items && selectedOrder.items.map((item, idx) => (
                          <div key={idx} className="flex flex-col sm:flex-row justify-between sm:items-center py-2.5 border-b border-cream3 last:border-0 gap-4 text-xs text-dark min-w-0">
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              {item.image && (
                                <img src={item.image} alt={item.name} className="w-10 h-12 object-cover bg-cream border border-cream3 rounded-lg shrink-0 hover:scale-105 transition-transform duration-300" />
                              )}
                              <div className="min-w-0 flex-1">
                                <span className="font-bold block uppercase leading-tight text-dark mb-1">{item.name}</span>
                                <div className="flex flex-wrap items-center gap-1.5 mt-1 font-mono text-[9px] font-bold text-dark/50">
                                  <span>SKU: {item.sku || item.id?.split('-')[0] || 'N/A'}</span>
                                  <span>•</span>
                                  <span className="text-dark font-black">SIZE: {item.size || 'M'}</span>
                                  {item.color && (
                                    <>
                                      <span>•</span>
                                ...
                                <span className="text-accent font-black font-mono text-[9px] uppercase tracking-wider mt-1 block">Quantity: {item.qty || 1}</span>
                              </div>
                            </div>
                            <div className="flex items-center justify-between sm:justify-end gap-4">
                              {(item.designId || item.customDesign) && (
                                <button
                                  ...
                                  className="inline-flex items-center gap-1.5 px-3 py-2 bg-dark hover:bg-accent text-white rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all duration-300 shadow-xs hover:shadow-md active:scale-[0.97] cursor-pointer whitespace-nowrap border-none"
                                >
                              ...
                              <div className="text-right shrink-0 font-mono">
                                <span className="text-dark/50 block text-[9.5px]">₹{(item.price || 0).toLocaleString('en-IN')} each</span>
                                <span className="font-black block text-xs">₹{((item.price || 0) * (item.qty || 1)).toLocaleString('en-IN')}</span>
                              </div>
                            </div>
                          </div>
                        ))}
                    ...
                    {/* Amount Breakdown */}
                    <div className="bg-white border border-cream3 rounded-2xl p-4 sm:p-5 space-y-2 font-mono text-[11px] text-dark shadow-xs">
                      <span className="text-[9.5px] text-dark2/45 uppercase font-black tracking-widest block border-b border-cream3 pb-2 mb-2">Invoice Summary</span>
                      ...
                      <div className="flex justify-between border-t border-cream3 pt-2.5 font-black text-xs sm:text-sm text-dark"><span>Grand Total</span><span className="font-mono text-sm">₹{selectedOrder.total.toLocaleString('en-IN')}</span></div>
                    </div>
                  ...
                  <div className="lg:col-span-5 space-y-5">
                    ...
                    {/* Key Metrics Grid */}
                    <div className="grid grid-cols-3 gap-2 border border-cream3 rounded-2xl p-3.5 bg-white shadow-xs font-mono text-[10px] sm:text-[11px] text-dark">
                      <div className="text-center sm:text-left border-r border-cream3 last:border-0 pr-1">
                        <span className="text-[8px] text-dark2/40 uppercase font-black block leading-none mb-1">Total</span>
                        <span className="font-black text-xs text-dark block truncate">₹{selectedOrder.total.toLocaleString('en-IN')}</span>
                      </div>
                      <div className="text-center sm:text-left border-r border-cream3 last:border-0 px-1">
                        <span className="text-[8px] text-dark2/40 uppercase font-black block leading-none mb-1">Qty</span>
                        <span className="font-black text-xs text-dark">{selectedOrder.items?.reduce((sum, item) => sum + (item.qty || 1), 0) || 0}</span>
                      </div>
                      <div className="text-center sm:text-left last:border-0 pl-1">
                        <span className="text-[8px] text-dark2/40 uppercase font-black block leading-none mb-1">Date</span>
                        <span className="font-bold text-dark block leading-tight text-[10px] mt-0.5">{formatOrderDate(selectedOrder.created_at)}</span>
                      </div>
                    </div>

                    {/* Customer Information */}
                    <div className="bg-white border border-cream3 rounded-2xl p-4 space-y-2.5">
                      <h4 className="text-[9px] font-mono uppercase font-black text-dark2/50 tracking-wider flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-accent" />Customer Details</h4>
                      <div className="space-y-2 bg-cream/30 p-2.5 rounded-lg border border-cream3/40">
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1">
                          ...
                          <button onClick={() => handleCopy(selectedOrder.email, 'email')} className="font-semibold text-left sm:text-right hover:text-accent flex items-center gap-1.5 border-none bg-transparent cursor-pointer break-all">
                            <span>{selectedOrder.email}</span>
                            {copiedField === 'email' ? <Check className="w-3 h-3 text-green-600 shrink-0" /> : <Copy className="w-2.5 h-2.5 text-dark2/40 shrink-0" />}
                          </button>
                        </div>
                        {selectedOrder.phone && (
                    ...
                    {/* Payment Details */}
                    <div className="bg-white border border-cream3 rounded-2xl p-4 space-y-2.5">
                      <h4 className="text-[9px] font-mono uppercase font-black text-dark2/50 tracking-wider flex items-center gap-1.5"><CreditCard className="w-3.5 h-3.5 text-accent" />Payment Method</h4>
                      <div className="space-y-2 bg-cream/30 p-2.5 rounded-lg border border-cream3/40">
                        ...
                        {rzpOrderId && (
                          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1">
                            ...
                            <button onClick={() => handleCopy(rzpOrderId, 'rzp')} className="font-mono text-[9px] font-bold text-dark/80 hover:text-accent truncate text-left flex items-center gap-1 border-none bg-transparent cursor-pointer break-all">
                              <span className="truncate max-w-[120px]">{rzpOrderId}</span>
                              {copiedField === 'rzp' ? <Check className="w-3 h-3 text-green-600 animate-bounce" /> : <Copy className="w-2.5 h-2.5 text-dark2/40" />}
                            </button>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="bg-white border border-cream3 rounded-2xl p-4 space-y-2.5">
                      <div className="flex justify-between items-center">
                        <h4 className="text-[9px] font-mono uppercase font-black text-dark2/50 tracking-wider flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-accent" />Shipping Address</h4>
                        <button onClick={() => handleCopy(selectedOrder.address, 'shipping')} className="text-[9px] font-black uppercase text-dark2/50 hover:text-accent flex items-center gap-1 bg-cream2 px-1.5 py-0.5 rounded border border-cream3 transition-all cursor-pointer">
                          {copiedField === 'shipping' ? <span>Copied!</span> : <span>Copy</span>}
                        </button>
                      </div>
                      <p className="bg-cream/30 p-2.5 rounded-lg border border-cream3/40 font-semibold leading-relaxed whitespace-pre-line text-[11px] text-dark/85">{selectedOrder.address}</p>
                    </div>
