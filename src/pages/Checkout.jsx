    ...
    try {
      ...
      try {
        ...
        for (const item of items) {
          ...
          if (foundProd) {
            await decrementProductStock(foundProd.id, item.qty || 1)
          }
        }
      } catch (stockErr) {
                  ...
                  return (
                    <div key={`${item.id}-${item.size}-${item.color || ''}`} className="py-4 flex gap-4 items-center">
                      ...
                      <div className="flex gap-1.5 flex-wrap items-center mt-0.5">
                        ...
                        {item.color && (
                          <span className="inline-flex items-center gap-1.5 text-[9px] text-dark2/40 font-mono font-bold uppercase">
                            <span 
                              style={{ backgroundColor: getColorHex(item.color) }} 
                              className="w-2 h-2 rounded-full border border-black/10 inline-block shrink-0" 
                            />
